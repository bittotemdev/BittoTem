...
unsigned int nStakeTargetSpacing = 1 * 60 * 1; // DIFF: 1-minute block spacing
int64 nChainStartTime = 1399120000;
int nCoinbaseMaturity = 30; //Block Maturity: 30+20 Confirms
...
const string strMessageMagic = "BittoTem Signed Message:\n";
...
// BittoTem: increasing Nfactor gradually
...
{
    static int64 nRewardCoinYear = 25 * CENT;  // creation amount per coin-year
    int64 nSubsidy;
    if(fTestNet)
...
static const int64 nTargetTimespan =  7 * 24 * 60 * 60;  // 7 days
...
unsigned int GetNextTargetRequired(const CBlockIndex* pindexLast, bool fProofOfStake)
{
 ...
 /*            if(fTestNet || (pindexLast->nHeight + 1 > 15000))
    ...
    if(pindexPrev->GetBlockTime() < 1398873600)
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "BittoTem", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        //  Coinbase(hash=2b345ac4d3, nTime=1396178760, ver=1, vin.size=1, vout.size=1, nLockTime=0)
        ...
        // Genesis block
        const char* pszTimestamp = "Bittotem created by Ray,1626907@gmail.com";
        CTransaction txNew;
        ...
        assert(block.hashMerkleRoot == uint256("0x0062d7cc82ab9e3787486a6ddc51600a759e00b836625a177b33082a6e400236"));
...
unsigned char pchMessageStart[4] = { 0x0f, 0x4a, 0x5f, 0x77 };
...
if(currentTimestamp < 1399120000)
        {
        ...
        else if(currentTimestamp < 1399120000)
        {
            if (!pfrom->fClient && !pfrom->fOneShot &&
                (pfrom->nStartingHeight > (nBestHeight - 144)) &&
                (pfrom->nVersion < NOBLKS_VERSION_START ||
