...
//    Litecoin and modified for BittoTem.
//
//    Parameters: lookup (int) - How many blocks to look into the past.
//    Returns: Estimated BittoTem network hash rate (uint64_t)
//
...
// WM - Implementation of getnetworkhashps for BittoTem
Value getnetworkhashps(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            "getnetworkhashps\n"
            "Returns an estimate of the BittoTem network hash rate.");
...
Value getworkex(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "BittoTem is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "BittoTem is downloading blocks...");
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "BittoTem is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "BittoTem is downloading blocks...");
